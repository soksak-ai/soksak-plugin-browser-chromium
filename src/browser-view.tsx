...
import { createBrowserToolbar, initialNavState } from "soksak-kit-browser-common";
import type { BrowserToolbar, NavState } from "soksak-kit-browser-common";
...
// 드래그 내내 추종 루프를 살려 surface bounds 를 매 프레임 커밋한다(DOM 과 실시간 일치).
...
function BrowserViewImpl({
  ...
  useEffect(() => {
    ...
    // 앵커 rect 로 surface bounds 를 커밋한다: DOM 분할은 이미 매 프레임 라이브 커밋이므로
    // surface 가 같은 리듬으로 따라와야 실시간 리사이즈다(최대 1프레임 지연). 변화 없는 프레임은
    // same-rect 스킵이라 IPC 0. 끝: 최종 레이아웃 커밋 후 도착하므로 최종 rect 로 1회 강제 스냅.
    // (freeze-frame 정지 사진·surface 숨김/복원은 폐기 — 드래그 중 콘텐츠가 박제되고, 옛 크기
    //  스탠드인이 빈 슬롯을 드러내는 잔상의 근원이었다.)
  ...
  useEffect(() => {
    ...
    const d1 = webview.on(label, "nav", (p) => {
      ...
      setLocalUrl(url);
      // title 폴백 — 탭 제목은 콘텐츠 사실: title 이벤트가 없는 페이지에서도 stale 제목이
      // 남지 않게 nav 시점에 URL(host 우선)로 먼저 보고, 진짜 title 이벤트가 오면 덮는다.
      if (url) {
        let t = url;
  ...
  // 공용 툴바 ↔ React 상태 동기(콜백은 매 렌더 최신으로, 표시값은 effect 로).
  tbCbRef.current = {
  ...
  useEffect(() => { tb?.setNavState(nav); }, [tb, nav]);
  ...
  return (
    <div className="browser-view">
      {/* 공용 툴바(soksak-kit-browser-common) 호스트 — 세 브라우저 동일 DOM·노드·외형. 고유 버튼은 extraSlot 포털. */}
